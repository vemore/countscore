...
show a finished game's score sheet. Also, all eight captures predate the visual refresh
(`wip/done/2026-09-18-app-looks-like-a-default-material-template.md`), so the store shows the
old look.
...
`scripts/compose_screenshots.py`. Re-capture all eight in the same device session, so the
carousel shows the refreshed UI.
