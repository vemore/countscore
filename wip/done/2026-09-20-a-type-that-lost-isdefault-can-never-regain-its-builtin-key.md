...
**Status:** done (2026-09-22) — closed by fix/builtin-key-rekey. Le schéma passe en **v19** : `applyV19` (`lib/services/sync/sync_schema.dart`, lancé par les deux moteurs) donne `builtin_key` à toute ligne vivante sans clé dont le nom stocké est — casse ASCII ignorée — le nom d'un seul type intégré **doté de règles** dans l'une des dix locales, son nom de semence ou son nom d'avant v14 (`builtinNamesByKey`), **pourvu qu'aucune ligne vivante ne tienne déjà cette clé** — une ligne par clé, la plus ancienne tous noms confondus ; `other`, sans règles et au nom générique, n'est jamais réclamé ; puis rejoue `applyV16`. Ni `isDefault` ni les colonnes de score ne sont lus, et seuls `builtin_key` et `rules_slug` sont écrits ; `isDefault` n'est pas remis. Deux constats ont corrigé l'entrée : l'historique git montre que seul **Skyjo** a été semé — « 6 qui prend » et « Yam's » ont été créés par l'utilisateur avant v6, dont le back-fill a tamponné le même `created_at` sur toutes les lignes, si bien que ce `created_at` ne prouve rien ; et les colonnes de score ne peuvent pas servir d'indice, la définition intégrée ayant bougé depuis (6 qui prend : 66 sur la ligne, 65 dans le code). Décision de l'utilisateur (2026-09-22) : adopter aussi « 6 qui prend », dont la clé était libre parce que v14 avait refusé d'insérer le type intégré à côté ; « Yam's » ne porte aucun nom intégré et la clé `yahtzee` est tenue par la ligne « Yahtzee » semée, donc il reste un type perso (critère 6 levé pour lui — il n'a aucune partie). Sur une copie de la sauvegarde du Pixel du 2026-09-19, la chaîne v14 → v19 donne `skyjo`/`skyjo` et `six_nimmt`/`six_nimmt`. `test/migration_v18_to_v19_test.dart` couvre les critères 1 à 4 sur les deux moteurs. [[SchemaV10]] (ligne v19, décision, statut sur la décision v18), [[Sync]], [[Testing]].
