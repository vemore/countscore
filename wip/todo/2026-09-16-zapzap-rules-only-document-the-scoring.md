...
context block of `ZAPZAP_SYSTEM_PROMPT` (`backend/app/services/zapzap_prompt.py:24-31`, now `KNOWN_GAME_TYPES["zapzap"]` in
`backend/app/services/analysis/game_rules.py:75-87` since #76, and less precise),
...
ZapZap project's `GAME_RULES.md` (outside this repository: `~/workspace/zapzap/GAME_RULES.md` on the author's machine,
found 2026-09-18). ZapZap is a
...
- Nothing in it contradicts `KNOWN_GAME_TYPES["zapzap"]` in `backend/app/services/analysis/game_rules.py:75-87`, which is vaguer than the rules; bring it in line if it says less.
