...
holds. The v19 step does the same with a `builtin_key` it gives back ([[SchemaV10]]): the
linked row pushes its new key and slug, and where the group already holds a live row with
that key the server answers `builtin_key_taken`, which the client resolves as above —
superseded, then a pull that relinks the local row to the group's. `test/sync/sync_store_test.dart` covers the five cases.
