...
| [[SchemaV10]] | Schema v19: the twelve tables, `rules`/`rules_slug` (21 rulesets, keyed on `builtin_key`), `builtin_key` and its live-unique index, what `isDefault` does and does not mean, sync bookkeeping and capture triggers, tombstones, the migration chain, the v17 dedupe of keyless built-in copies, the v18 replay that restores a wiped slug, and the v19 step that gives a lost `builtin_key` back | 2026-09-22 |
