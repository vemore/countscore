...
| `test/screens/game_board_wide_layout_test.dart` | The board's score grid on either side of the 600 dp breakpoint: at 400 dp it keeps its intrinsic width, every column's `columnWidth` unset, and eight players still scroll sideways; at 1000 dp it is exactly the screen's width, the round column intrinsic and the player columns sharing the rest equally; twelve players at 1000 dp scroll rather than overflow. The view size is set on `tester.view`, at a device pixel ratio of 1. |
