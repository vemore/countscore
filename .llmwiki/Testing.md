...
| `test/migration_v17_to_v18_test.dart` | `applyV18` — the replay of `applyV16` — through both engines' real upgrade callbacks from a v17 file damaged the way the pre-1.3.1 editor damaged it (`rules_slug`, `rules` and `isDefault` cleared on rows that kept their `builtin_key`): ZapZap and 6 qui prend get their shipped ruleset back; a row whose slug was wiped but whose `rules` the user wrote keeps that text and gains only the slug; a renamed type and a user-built type, both keyless, stay untouched; a slug never wiped is not rewritten; `isDefault` is **not** restored; nothing is inserted and a replay changes nothing. Each assertion carries the acceptance criterion it covers. |
| `test/migration_v18_to_v19_test.dart` | `applyV19` through both engines' real upgrade callbacks from the owner's v18 shape: a seeded Skyjo that lost its key, a user's "6 qui prend" older than the built-in (with house rules and a threshold of 66) and a row stored under its Russian name all get key and slug back; `rules` and the scoring stay; "Yam's", an upper-cased homonym of a live built-in, a renamed built-in, a deleted homonym, a second Skyjo, a younger "Wizard" beside an older row stored in Russian and a user's own "Other" stay keyless; only `builtin_key` and `rules_slug` change, nothing is inserted, a replay changes nothing, `isDefault` stays 0; `builtinNamesByKey` covers exactly the keys of `defaultRulesSlugs` and knows 6 qui prend in all ten locales. |
...
| `test/migration_v5_to_v10_test.dart` | The production upgrade: a real v5 file from tag `1.0.1+3`'s DDL, upgraded with the production callbacks (`DatabaseService.openForTesting`), then read back through Drift — games, merged players, scores, stats — and written to, `finishedAt` included. Its name understates its range: it asserts `DatabaseService.schemaVersion`, so it runs v5 → the current version today and follows every bump without an edit. |
| `test/drift/drift_repositories_test.dart` | Full lifecycle through the Drift repositories over `AppDatabase.forTesting(NativeDatabase.memory())`; Drift `onCreate` builds the v10 tables; shared rows are tombstoned (game, round, membership, `deleteByName`), local ones deleted, and tombstones count in no statistic; `finishedAt` survives create, update and reopen — the only test that catches a field missing from `update`'s hand-written column list. It also proves the statistics place a finished elimination game exactly as `GameStanding.forGame` does, and that a race to a total and a game with no type keep the score order; counts the games of a type and the finished ones among them (what the editor asks before a deletion or a win-direction flip), proves an edit built with `copyWith` carries `rules`, `rulesSlug` and `isDefault` across, and pins `GameType.toMap()`'s column set so a new column cannot be added without deciding what the editor does with it. |
...
| `test/widgets/group_settings_section_test.dart` | Settings → Group pumped with asserts on: create a group through the dialog against a `MockClient` server, and the group and its invite code appear; removing a lost device shows the rotated invite code; the join and create dialogs put the nickname first, empty, keep OK disabled while any field is blank (whitespace included) and hand `joinGroup` / `createGroup` the trimmed values (a fake `GroupProvider`); the nickname edit in the section sends the trimmed name to `PATCH /groups/devices/me` and shows it, and sends nothing when the name is unchanged; the fields stop at 64 code points. Guards the dialog that disposed its controllers during its exit transition (`_dependents.isEmpty`, found on a Pixel on 2026-09-13, invisible in release builds). |
...
| `test/screens/game_board_end_of_game_test.dart` | The board's end of game: the finished chip appears and disappears with `finishedAt`, and **Round N** is disabled while it is set (back after Reopen or "Continue playing"), and the game type's rule finishes the game and opens the end screen — naming the winner on a highest-wins and on a lowest-wins type — after a round is validated on the keypad, keeps quiet for a crossing already answered "Continue playing" (which returns to the board with the game reopened; back leaves it finished), re-arms once the game is back under its threshold, notices a crossing made outside the board at once (the `GameProvider` listener), does not re-end a game another device finished and then reopened (pulled finish, reopen, round 3: the game stays open), keeps the end screen from opening over an open keypad and drops a round validated after the game ended, is raised once on the board's first build for an open game past its threshold and not for a finished one, and remembers "Continue playing" across leaving the board (SharedPreferences, cleared when the game goes back under); the app bar carries exactly one way to the standings (`board_standings`), which opens them with the winner's headline on a finished game and without it on an open one, and never with "Continue playing". A seeded ZapZap ends on the round that leaves one player standing (not on one that leaves two), and ends too when that round arrives through `GameProvider.refreshFromSync`, as a pull delivers it, on a board that did not type it; a pull that changes no total asks nothing. Président and Uno (`firstPlayerOver`) still end on reaching 10 and 500, not one point before. `GameBoardScreen.analysisRepo` is injected for the same reason `GameProvider`'s repositories are — the default reaches the singleton. |
...
| `test/models/game_standing_test.dart` | The ranking rule (`GameStanding.forGame`): the reproduced ZapZap game ranks by the elimination order once finished and by the total while open; a race to a total, a type with no rule and no type at all rank by the total; the total breaks a tie between two players out in the same round, and two survivors tied share a place; a player who never scored is not out; no score at all crowns nobody; `rankedPlayers` keeps the seat order on a tie. `lastPlayerUnder` is the mirror of the `over` case, round for round and place for place; `firstPlayerUnder` is a race; and of the twenty-two seeded types exactly `zapzap`, `rami` and `six_nimmt` depart from the score order — the guard that no seeded standing moves. |
| `test/models/player_stats_test.dart` | The leaderboard and the player card over `FinishedGameResult`s: ties share a place and a tie for first is a win for both, five games to be ranked, game types by play count, colours from the latest game, the card's average place, streak, trend and best total. A finished elimination game is placed by its `RankingRule` — the reproduction's wins, average place and rank chart follow the standings' order, not the totals'. |
...
| `test/providers/group_provider_nickname_test.dart` | `GroupProvider.deviceLabel` against a `MockClient`: a devices list that left before a rename does not bring the old name back, and a nickname missed at an offline start is read after the next successful sync. |
| `test/services/backend_client_test.dart` | `BackendException` carries the status, keeps the body for logging, and decodes utf8 on both the error and the success path; a 404 on `/comments/game-analysis` is retried once on the legacy path, and a 404 from both is still a failure; the group's device list sends the device token, revoking another device returns the rotated invite code, and revoking this one is leaving (204, no code); `renameDevice` patches `/groups/devices/me` with the token and returns the stored label. `MockClient` from `package:http/testing.dart`. |
...
| `test/widgets/dice_roller_dialog_test.dart` | With a seeded `Random`, for every count 1..6 and ten re-rolls each: as many dice as chosen, every value 1..6, the total shown is their sum, every face turns up; the same seed rolls the same dice. Plus a layout case run at 412×915 and at 1600×900 with `devicePixelRatio` 1: opened through `showDialog`, the six count chips share one top edge, run left to right, and stay on the dialog's surface — the laid-out geometry, because six chips that exist say nothing about the row that broke ([[MobileApp]], *Decisions & History*). |
...
| `test/screens/standings_screen_test.dart` | The one standings screen in both states (it merged `game_end_screen_test.dart` and `ranking_screen_test.dart` on 2026-09-20). Open: board colours, the sole leader crowned, the *Ranking* title, no headline and no *Analysis* even with a server. Finished: the winner named, the podium's three places with their totals, *Analysis* absent without a server (Play again then spans the row) and present with one, and "End game" from the home card menu finishing the game and opening the screen. Common: the list under the podium holds every player, first to last, for 2, 3, 4 and 8 players and does not scroll at 412×860 for four; finishing a game does not change the order; no crown with no round played, after a round of all zeros, or on a tie, where the tied players stand on the same step and share a place number (1, 1, 3); the share action hands the same order to the sheet as text and as the `ResultShareCard` PNG, and a sheet that fails to open is reported. A finished elimination game — the ZapZap reproduction of `wip/done/2026-09-20-ranking-ignores-the-game-types-ranking-rule.md` — ranks by the elimination order on screen, in the shared text and through `standingOf`, the home cards' source; the same game open, and the same game of a race-to-a-total type, rank by the total. |
| `test/screens/create_game_screen_test.dart` | The New game screen at 412 dp: six game-type tiles three a row, the last game's type first; the name after the last game's; the rule line; the seats under it in the last game's order, the first one dealing; a full-width *Start · N players* at the bottom. A player's avatar colour here equals his colour on the real board, also when two players own the same colour; a seat dragged by its handle changes the created game's `orderIndex`; the "who's playing" sheet creates a player and seats him after the ones checked; "All games" puts a type that was not on a tile onto one; "Other" offers the win rule; the first game is named in the app's language (`en`, `fr`, `ja`); with no game type at all it shows "No game types" (`game_type_empty`), not an endless "Loading game types…". With the name field focused, closing the players sheet or the *All games* sheet leaves no `EditableText` focused and the keyboard hidden. The board is injected (`boardBuilder`). |
...
| `test/screens/game_types_screen_test.dart` | At 412×860 with twenty types, scrolled to the end, the last type's ⋮ menu does not overlap the "New type" button, is hit-testable and opens; the new-type dialog's icon and colour buttons carry the `chooseIcon` and `chooseColor` tooltips as their semantics. Then the editor itself: an edit keeps `rules`, `rulesSlug`, `isDefault` and `builtinKey`; *Aucune* clears a condition *and* its threshold; an empty name marks the field with no snackbar; the name is trimmed and the field capped at 64; either threshold is required once its condition is chosen, refuses letters and a value over the maximum; no controller outlives the dialog (a disposed one refuses a listener); the icon grid marks the current icon; a colour outside the palette is offered as the current swatch; a used type is refused with a French ICU plural and no `Exception`; an unused one is confirmed as a *type*; flipping the win direction asks first under finished games and leaves the row untouched when refused, saves silently without them, and flags a contradicting game-over condition without blocking; a changed condition on a type with rules offers the rules editor, a colour change does not. The dialog tests run at 1000×1400 because the test font overflows its dropdowns at phone width. |
| `test/utils/game_type_appearance_test.dart` | `kGameTypeIcons` holds every icon `GameType.defaultGameTypes()` seeds (a seeded icon the picker cannot reach makes that type's icon a one-way door), and every `kGameTypePalette` swatch clears a 3.0 contrast ratio against both themes' surfaces and cards, tinted at 0.10 and 0.15 as the screens draw them. |
...
`tests/test_groups.py` (create, join, revoke, rotate, a device renaming itself) · `test_sync_contract.py` (batch
