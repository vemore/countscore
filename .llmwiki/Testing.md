...
| `test/migration_v12_to_v13_test.dart` (7) | Hand-written v12 `game_types` fixture through `applyV13`: the ten seeded rows back-filled by name, the twelve new types inserted, a deleted type not resurrected, a renamed one left keyless, two same-named rows yielding one key, and a replay changing nothing. |
| `test/utils/game_type_name_test.dart` (6) | Every `builtinKey` has a case in the display-name switch; a built-in name follows the locale whatever the row stores; a user type and an unknown key fall back to the stored name. The switch is explicit because `AppLocalizations` has no lookup by name, so only this test catches a key added without its string. |
...
| `test/drift/web_upgrade_test.dart` (1) | A v9 database (v10/v11/v12/v13 stripped, `user_version` 9) reopened through Drift gets the sync tables, columns, triggers, `games.finishedAt` and the back-filled `game_types.builtin_key` from `onUpgrade` — the PWA's upgrade path, and the only engine that runs it. |
| `test/widget_test.dart` (10) | Model serialisation only — it pumps no widgets, despite the name. It pins the built-in game types **by index**: the first ten are what a pre-v13 install already holds, so a new type is appended, never inserted. |
