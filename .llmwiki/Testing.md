...
| `test/drift/web_upgrade_test.dart` | A v9 database (v10/v11/v12/v13/v14 stripped, `user_version` 9) reopened through Drift gets the sync tables, columns, triggers, `games.finishedAt` and the back-filled `game_types.builtin_key` from `onUpgrade` — the PWA's upgrade path, and the only engine that runs it. |
