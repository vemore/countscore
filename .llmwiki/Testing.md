...
| `test/drift/drift_repositories_test.dart` | Full lifecycle through the Drift repositories over `AppDatabase.forTesting(NativeDatabase.memory())`; Drift `onCreate` builds the v10 tables; shared rows are tombstoned (game, round, membership, `deleteByName`), local ones deleted, and tombstones count in no statistic; `finishedAt` survives create, update and reopen — the only test that catches a field missing from `update`'s hand-written column list. It also counts the games of a type and the finished ones among them (what the editor asks before a deletion or a win-direction flip), proves an edit built with `copyWith` carries `rules`, `rulesSlug` and `isDefault` across, and pins `GameType.toMap()`'s column set so a new column cannot be added without deciding what the editor does with it. |
...
| `test/models/game_standing_test.dart` | The ranking rule (`GameStanding.forGame`): the reproduced ZapZap game ranks by the elimination order once finished and by the total while open; a race to a total, a type with no rule and no type at all rank by the total; the total breaks a tie between two players out in the same round, and two survivors tied share a place; a player who never scored is not out; no score at all crowns nobody; `rankedPlayers` keeps the seat order on a tie. |
