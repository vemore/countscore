// The v13 → v14 step: `game_types.builtin_key`.
//
...
// three promises applyV14 makes: the ten seeded rows are back-filled by name,
...
/// `game_types` as of v13: everything but `builtin_key`.
Future<void> _createV13GameTypes(Database db) async {
  await db.execute('''
...
void main() {
  ...
  setUp(() async {
    db = await databaseFactoryFfi.openDatabase(
      ...
      options:
          OpenDatabaseOptions(version: 13, onCreate: (db, _) => _createV13GameTypes(db)),
    );
  });
  tearDown(() => db.close());

  Future<void> upgrade() => applyV14(db.execute, (t) => _columnsOf(db, t));

  test('the ten seeded rows are back-filled by the name they were seeded with', () async {
    for (final name in GameType.seededNamesBeforeV14.values) {
      await _insertV13(db, name);
    }

    await upgrade();

    final keys = await _keysByName(db);
    for (final seeded in GameType.seededNamesBeforeV14.entries) {
  ...
  test('a type the user renamed stays a user type', () async {
    await _insertV13(db, 'Le jeu du jeudi'); // was Skyjo, renamed
    await _insertV13(db, 'ZapZap');
  ...
  test('two seeded rows with the same name yield one key, the oldest row', () async {
    await _insertV13(db, 'Uno');
    await _insertV13(db, 'uno', uuid: 'uuid-uno-2');
  ...
  test("a type the user made themselves is never claimed, nor duplicated", () async {
    ...
    await _insertV13(db, 'ZapZap');
    await _insertV13(db, 'Yahtzee', isDefault: false);
  ...
  test('a seeded row the user renamed frees its name for the new type', () async {
    ...
    await _insertV13(db, 'Le jeu du jeudi');
  ...
  test('a tombstoned built-in row does not block its key from being re-seeded',
    ...
    await _insertV13(db, 'Autre', deletedAt: 99);
