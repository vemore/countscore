#!/usr/bin/env bash
# Verify a signed CountScore App Bundle before it goes anywhere near the Play Console.
#
#   .claude/skills/release-android/scripts/verify_aab.sh [path/to/app-release.aab]
#
# Run from the checkout that built the bundle. Read-only: it never prints a password and
# never modifies the bundle. Exits non-zero on the first failed check.
...
MANIFEST="$ROOT/build/app/intermediates/merged_manifests/release/processReleaseManifest/AndroidManifest.xml"
MIN_TARGET_SDK=36
...
[[ "$owner" != *"Android Debug"* ]] || fail "signed with the DEBUG key ($owner) — key.properties was not picked up"
aab_sha="$(sed -n 's/^[[:space:]]*SHA256: //p' <<<"$certs" | head -n1)"
[[ -n "$aab_sha" ]] || fail "no SHA-256 fingerprint in the bundle certificate"

[[ -f "$PROPS" ]] || fail "android/key.properties missing — cannot compare against the upload key"
store="$(prop storeFile)"; alias="$(prop keyAlias)"
[[ "$store" = /* ]] || store="$ROOT/android/$store"
...
unzip -p "$AAB" base/manifest/AndroidManifest.xml | grep -a 'android.permission.INTERNET' >/dev/null \
  || fail "the bundle manifest does not declare INTERNET — analysis and group sharing will fail"
...
[[ "$MANIFEST" -nt "$ROOT/pubspec.yaml" && ! "$MANIFEST" -nt "$AAB" ]] \
  || fail "merged manifest is stale relative to pubspec.yaml or the bundle — rebuild"
code="$(grep -o 'android:versionCode="[0-9]*"' "$MANIFEST" | grep -o '[0-9]\+')"
target="$(grep -o 'android:targetSdkVersion="[0-9]*"' "$MANIFEST" | grep -o '[0-9]\+')"
pub_code="$(sed -n 's/^version: *[^+]*+\([0-9]*\).*/\1/p' "$ROOT/pubspec.yaml")"
[[ "$code" == "$pub_code" ]] || fail "versionCode $code does not match pubspec.yaml build number $pub_code"
ok "versionCode $code (pubspec.yaml $(sed -n 's/^version: *//p' "$ROOT/pubspec.yaml"))"
